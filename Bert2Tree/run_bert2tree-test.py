...
def set_args():
    ...
    # 预训练模型路径    
    parser.add_argument('--bert_path', type=str, default="./data")
...
if __name__ == "__main__":
    ...
    logfile = args.save_path + '/log_continue'
    ...
    for epoch in range(args.n_epochs):
        ...
        #     file.write("train_loss:", str(loss_total / len(train_data)), "  train_accurate:", str(num_accurate/len(train_pairs)) + "\n")
        ...
        if (epoch+1) % valid_epoch == 0 or epoch > args.n_epochs-5:
            ...
            for test_batch in tqdm(test_pairs):
                ...
                test_res = evaluate_tree(generate_num_ids, encoder, predict, generate, merge, 
                    ...
                    token_len, beam_size = args.beam_size)
                val_ac, equ_ac, _, _, test_num, tar_num = compute_test(test_res, test_batch["output"], 
                    output_lang, test_batch["nums"])
                if val_ac:
                    value_ac += 1
                    save_case.append([1, test_num, tar_num])
                else:
                    save_case.append([0, test_num, tar_num])
                if equ_ac:
                    equation_ac += 1
                test_total += 1
            ...
            predict_case = save_case
            label = read_json(args.test_path)
            true = 0
            for each_pre,each_label in zip(predict_case, label):
                if each_pre[1] != None:
                    if abs(each_pre[1]-float(each_label["ans"]))<1e-1:
                        true += 1
            value_ac = true
    ...
    if args.save:
        ...
        print("------------------------------------------------------")
        with open(args.save_path +'/case.json', 'w', encoding='utf-8') as file_obj:
            json.dump(save_case, file_obj, ensure_ascii=False, indent=2)
