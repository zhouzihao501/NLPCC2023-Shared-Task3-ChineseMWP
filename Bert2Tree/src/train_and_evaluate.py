...
def compute_test(test_res, test_tar, output_lang, num_list):
    test = out_expression_list(test_res, output_lang, num_list)
    tar = out_expression_list(test_tar, output_lang, num_list)
    
    try:
        test_num = compute_prefix_expression(test)
    except:
        test_num = None
    tar_num = compute_prefix_expression(tar)
    
    if test_res == test_tar:
        return True, True, test_res, test_tar, test_num, tar_num


    if test is None:
        return False, False, test, tar, test_num, tar_num
    if test == tar:
        return True, True, test, tar, test_num, tar_num
    try:
        if abs(compute_prefix_expression(test) - compute_prefix_expression(tar)) < 1e-4:
            return True, False, test, tar, test_num, tar_num
        else:
            return False, False, test, tar, test_num, tar_num
    except:
        return False, False, test, tar, test_num, tar_num
